...
func TestBuildConfigOptions(t *testing.T) {
	...
	if configOptions.ConfigFile != filepath.Join(testHomeDir, ".config", "keeper", "aws-credential-process.json") {
		t.Errorf("ConfigFile not built correctly")
	}
	if configOptions.ConfigFileBackup != filepath.Join(testHomeDir, "aws-credential-process.json") {
